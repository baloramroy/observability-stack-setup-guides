...
```bash
sudo mv grafana_12.3.0 grafana
```
...
So from that directory we copy the `sample.ini` file and rename it `grafana.ini` config file and will change necessary adjustments.
...
ExecStart=/usr/local/grafana/bin/grafana-server --config="${GF_PATHS_CONFIG}" --homepath=/usr/local/grafana
Restart=on-failure
LimitNOFILE=65536
...
If you want, I can also generate a **PDF**, **HTML**, or **Confluence-style** document from this.
