...
### Ensure Rule Files Are Loaded
  ...
  Check defined rules file are loaded or not in `prometheus.yml` file:

  ```yaml
  rule_files:
    - "alert_rules/*.yml"
  ```
  > It this configuration is loaded in **prometheus** config file then create a **temporary alert** rules and check alerts are **visible** or not.
#
...
- Check `alerting.alertmanagers.targets` by navigating **Status -> Configuration** in Promethues web UI.
